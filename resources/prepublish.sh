...
# Publishing to NPM is currently supported by Circle CI, which ensures that all
...
if [ "$CI" != true ]; then
  echo "\n\n\n  \033[101;30m Only Circle CI can publish to NPM. \033[0m" 1>&2;
...
# Build before Circle CI publishes to NPM
npm run build;
